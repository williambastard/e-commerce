...
export function ProductCard({ product }: { product: productInterface }) {
  return (
    <section
      ...
      className="m-2 w-[15rem] self-end rounded-sm border bg-(--color-couleur-secondaire) p-4 shadow-md"
      aria-label={`${product.title}`}
    >
      <div className="mb-2 h-5 content-center justify-self-end">
        {product.discount! > 0 && (
          <div className="rounded-md bg-(--color-couleur-accent-7) pr-2 pl-2 text-center text-xs font-bold">
            <span>Promotion -{product.discount}%</span>
          </div>
        )}
      </div>
      ...
      <div className="p-2">
        <div className="mt-2 flex items-start justify-between pb-6">
          <div className="overflow-hidden">
            <h3
              itemProp="name"
              className="xs:text-xs line-clamp-1 text-sm font-bold text-(--color-couleur-accent-5) lg:text-xl"
            >
              {product.title}
            </h3>
            <p
              itemProp="description"
              className="xs:text-xs lg:text-md line-clamp-3 text-sm text-gray-600"
        ...
        <div className="m-2 flex items-center justify-between">
          <div className="xs:text-xs lg:text-md flex flex-col items-start text-sm">
            <span className="text-(--color-background) italic">Prix T.T.C</span>
            <div
              itemProp="offers"
              itemScope
              itemType="http://schema.org/Offer"
              className="font-bold text-(--color-background)"
            >
          ...
          {product.stock > 0 && (
            <button className="rounded-sm bg-(--color-couleur-primaire) px-4 py-2 font-bold text-white hover:cursor-pointer hover:bg-(--color-cta-red-hover)" aria-label={`Acheter l'article ${product.title}`}>
              <svg
